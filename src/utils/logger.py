...
class Log:
    ...
    def req(self, user, request, command):
        ...
        if self.print_to_console:
            print(
                f'{datetime.now().replace(microsecond=0)} - {self.platform} - Command: {command}, User: {user}, '
                f'Request: {request}'
            )
    ...
    def info(self, info):
        ...
        if self.print_to_console:
            print(f'{datetime.now().replace(microsecond=0)} - {self.platform} - {info}')
