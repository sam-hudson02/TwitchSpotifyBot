...
class DiscordCreds:
    ...
    def token(self):
        if self.__token is None:
            self.log.critical('Discord token missing! Not continuing.')
            raise NoCreds
        return self.__token
    ...
    def queue_channel_id(self):
        if self.__queue_channel_id is None:
            self.log.critical('Discord queue channel id missing! Not continuing.')
            raise NoCreds
    ...
    def leaderboard_channel_id(self):
        if self.__leaderboard_channel_id is None:
            self.log.critical('Discord leaderboard channel id missing! Not continuing.')
            raise NoCreds
...
class SpotifyCreds:
    def __init__(self, log: Log):
        ...
        self.__client_secret = getenv('SPOTIFY_CLIENT_SECRET')
