...
from server.models import ActiveUpdate, ControlResponse, TwitchStatus
...
async def start_twitch(state: AppState = Depends(get_state)):
    if state.twitch_running:
        return ControlResponse(
            service='twitch', running=True, message='already running'
        )
...
@router.put(
    '/active', response_model=ActiveUpdate, dependencies=[Depends(require_auth)]
)
