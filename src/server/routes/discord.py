...
from server.models import ControlResponse, DiscordStatus
...
async def discord_status(state: AppState = Depends(get_state)):
    ...
    return DiscordStatus(
        connected=state.discord_running,
        queue_webhook=creds.queue_webhook is not None,
        leaderboard_webhook=creds.leaderboard_webhook is not None,
    )


@router.post(
    '/start', response_model=ControlResponse, dependencies=[Depends(require_auth)]
)
async def start_discord(state: AppState = Depends(get_state)):
    if state.discord_running:
        return ControlResponse(
            service='discord', running=True, message='already running'
        )
    creds = state.creds.discord
    if not (creds.queue_webhook or creds.leaderboard_webhook):
        raise HTTPException(status_code=409, detail='No Discord webhooks configured')
...
@router.post(
    '/stop', response_model=ControlResponse, dependencies=[Depends(require_auth)]
)
