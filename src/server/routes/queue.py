...
from server.models import QueueAdd, QueueItem, QueueMove
...
@router.post(
    '/queue', response_model=list[QueueItem], dependencies=[Depends(require_auth)]
)
async def add_to_queue(body: QueueAdd, state: AppState = Depends(get_state)):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail='Only Spotify tracks are supported')
    except TrackAlreadyInQueue:
        raise HTTPException(status_code=409, detail='That song is already in the queue')
    return await state.queue_snapshot()
...
async def move_in_queue(
    req_id: int, body: QueueMove, state: AppState = Depends(get_state)
):
    try:
