...
class TestMessageParsing(unittest.TestCase):
    def test_emotes_tag_with_colons(self):
        ...
        raw = (
            '@badge-info=;badges=broadcaster/1;display-name=Sam;'
    ...
    def test_colon_in_message_body(self):
        raw = (
            '@id=x;room-id=1;tmi-sent-ts=1;display-name=A;badges=;emotes=;'
            'user-id=2;user-type= :a!a@a.tmi.twitch.tv PRIVMSG #chan '
            ':check https://open.spotify.com/track/abc'
        )
        msg = make(raw)
        self.assertEqual(msg.content, 'check https://open.spotify.com/track/abc')

    def test_plain_message(self):
        raw = (
            '@id=y;room-id=1;tmi-sent-ts=1;display-name=B;badges=;emotes=;'
