...
class TestCommandConfig(unittest.TestCase):
    ...
    def test_on_off_keys_are_strings(self):
        ...
        cfg = self._config()
        self.assertEqual(cfg.message('DEV_ON', 'on'), 'Dev mode has been turned on!')
    ...
    def test_user_override_keywords_and_message(self):
        ...
        self.assertEqual(
            cfg.message('SONG_REQUEST', 'added', song='x', artist='y'), 'added x'
        )
        # unspecified message falls back to the default
        self.assertEqual(
            cfg.message('SONG_REQUEST', 'not_found'),
